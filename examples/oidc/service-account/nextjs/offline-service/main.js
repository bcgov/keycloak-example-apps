...
const main = async () => {
    ...
    postData.append('client_id', process.env.KEYCLOAK_ID)
    postData.append('client_secret', process.env.KEYCLOAK_SECRET)
    const { access_token: accessToken } = await fetch(`${process.env.KEYCLOAK_ISSUER}/protocol/openid-connect/token`, {
        method: 'POST',
        headers: {
